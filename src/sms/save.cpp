...
static void flush_now(void){
  ...
  size_t w = fwrite(g_sram, 1, g_sram_len, f);  // write all
  fflush(f); // force write
  fsync(fileno(f)); // actually push to SD card
  fclose(f);
...
void sms_save_tick(void){
  ...
  if (crc != g_crc_last && now >= g_next_allowed_write){
    ...
    if (!sram_is_trivial(g_sram, g_sram_len)) {
      flush_now();
      g_next_allowed_write = now + pdMS_TO_TICKS(GAP_MS);
    } else {
