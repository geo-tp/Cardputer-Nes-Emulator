...
pce_reset(bool hard)
{
	...
	if (PCE.VDC.regs == NULL)
		PCE.VDC.regs = (UWord*)malloc(32 * sizeof(UWord));
	if (PCE.VCE.regs == NULL)
		PCE.VCE.regs = (UWord*)malloc(0x200 * sizeof(UWord));
	if (PCE.Palette == NULL)
		PCE.Palette = (uint8_t*)malloc(512 * sizeof(uint8_t));
		
	memset(PCE.VCE.regs, 0, 0x200 * sizeof(UWord));
	memset(PCE.VDC.regs, 0, 32 * sizeof(UWord));
...
pce_init(void)
{
	...
	PCE.SPRAM = malloc(64 * sizeof(sprite_t));
	PCE.IOAREA = PCE.NULLRAM + 4;
	PCE.MemoryMapR = calloc(256, sizeof(uint8_t *));
	PCE.MemoryMapW = calloc(256, sizeof(uint8_t *));

	if (PCE.Palette == NULL) PCE.Palette = (uint8_t*)malloc(512 * sizeof(uint8_t));
	if (PCE.VCE.regs == NULL) PCE.VCE.regs = (UWord*)malloc(0x200 * sizeof(UWord));
	if (PCE.VDC.regs == NULL) PCE.VDC.regs = (UWord*)malloc(32 * sizeof(UWord));

	if (!PCE.RAM || !PCE.VRAM || !PCE.NULLRAM ||
	    !PCE.SPRAM || !PCE.Palette ||
	    !PCE.MemoryMapR || !PCE.MemoryMapW ||
	    !PCE.VCE.regs || !PCE.VDC.regs) {
